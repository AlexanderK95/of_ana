...
def derivative(arrX, arrY):
    out = numpy.zeros(arrX.size)
    for i in numpy.arange(1,arrX.size-1):
...
def saccade_detection(x, y, time, missing=0.0, minlen=5, maxvel=40, maxacc=340):
	...
	dx = derivative(time, x)
	dy = derivative(time, y)
	...
	vel = numpy.sqrt(dx**2 + dy**2)

	acc = derivative(time, vel)
	...
	while not stop:
		...
		if len(sacstarts) > 0:
			...
			sacends = numpy.where((vel[t1i:] < maxvel).astype(int) + (acc[t1i:] < maxacc).astype(int) == 2)[0]
			if len(sacends) > 0:
